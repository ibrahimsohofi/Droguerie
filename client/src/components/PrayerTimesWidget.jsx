  ...
  const fetchPrayerTimes = async () => {
    try {
      ...
      const response = await fetch(
        `https://api.aladhan.com/v1/timingsByCity?city=${city}&country=${country}&method=12`
      );
    ...
    return (
      <div className="relative">
        <button
          ...
          className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-white hover:text-orange-500 hover:bg-blue-800 transition-all duration-300 ${isRTL ? 'space-x-reverse' : ''}`}
        >
          <Clock className="h-5 w-5" />
          {nextPrayer && (
            <span className="text-sm font-medium hidden md:inline">
              {prayerNames[language][nextPrayer.name]} • {Math.floor(nextPrayer.timeUntil / 60)}h {nextPrayer.timeUntil % 60}m
            </span>
          )}
        </button>
        ...
        {isExpanded && (
          <>
            ...
            <div className={`absolute top-full ${isRTL ? 'left-0' : 'right-0'} mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-20`}>
              <PrayerTimesContent
