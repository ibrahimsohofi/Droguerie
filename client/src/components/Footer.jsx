...
const Footer = () => {
  ...
  const footerLinks = {
    quickLinks: [
      ...
      { name: language === 'ar' ? 'من نحن' : 'À propos', path: '/about' },
      { name: language === 'ar' ? 'الأسئلة الشائعة' : 'FAQ', path: '/faq' }
    ],
    categories: [
      { name: t.categories.cleaning, path: '/categories/1' },
      { name: t.categories.personal, path: '/categories/2' },
      { name: t.categories.cosmetics, path: '/categories/3' },
      { name: t.categories.household, path: '/categories/4' }
    ]
  };
